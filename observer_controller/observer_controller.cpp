...
static const unsigned int numLQRCosts = 2;

static double integratedLQRcosts[numLQRCosts] = {0.0};

static double costFunc(const double*, const double*);
static double* LQRcosts(double* x, double*u, double* costVector);
...
int main(int argc, char** argv)
{
	...
	case 6: //learning at balance
		...
		{
			double initialGains[]={-1.3, 0.477, -0.86, 0.72};
			balanceLearner = *(new LaBcontroller(costFunc,initialGains, dt, 4));
			//set learning params here if default is bad
		}
		break;

	case 7: //alternative learning at balance (non-REINFORCE)
		isLearning = true;
		learningRecord = fopen("learningRecord.dat", "w");
		printf("Executing alternative learning at balance control...\n\n");
		{
			double initialGains[]={-1.3, 0.477, -0.86, 0.72};
			balanceLearner = *(new LaBcontroller(costFunc,initialGains, dt, 4));
			balanceLearner.useAlternativeUpdating();
		}
		break;

	case 8: //alternative learning at balance (non-REINFORCE) w/ y learning
		isLearning = true;
		learningRecord = fopen("learningRecord.dat", "w");
		printf("Executing alternative learning at balance control w/ Y learning...\n\n");
		{
			double initialGains[]={-1.3, 0.477, -0.86, 0.72, 0.0};
			balanceLearner = *(new LaBcontroller(costFunc,initialGains, dt, 5));
			balanceLearner.useAlternativeUpdating();
			balanceLearner.useInternalCostFunc();
		}
		break;
	...
	while(keepRunning)//begin control loop
	{
		...
		case 2: //balancing operation
			...
			{ //integrate cost
				...
				LQRcosts(x, &myComm.commandValue, integratedLQRcosts);
			}
			break;
		...
		case 8:
			myComm = balanceLearner.getBalancingCommandwYLearning(state);
...
static lcmt_servotubeCommand balancingController(double y, double theta, double yd, double thetad)
{
	...
	/*
	double p[4] = {-5.03, 0.818, 2.560, 0.168};
	double alpha = 3.0;
	double Kyp = -1.8 + p[0]*alpha, Kyd = -.9 + p[2]*alpha; //p and d gains for y
	double Ktp = .21 + p[1]*alpha, Ktd = 0.32 + p[3]*alpha; //p and d gains for theta
	*/

	double Kyp = -10.0, Ktp = 20.88, Kyd = -0.099, Ktd = 7.185;
	//double Kyp = -10, Ktp = 18.1, Kyd = -15.9, Ktd = 7.3;
												   ...
												   double thetad, double yr, double ydr)
{
	...
	//double Kyp = -10.0, Ktp = 20.88, Kyd = -0.099, Ktd = 7.185;
	double Kyp = -36.51, Ktp = 24.52, Kyd = -8.80, Ktd = 6.39; 
...
static double* LQRcosts(double* x, double*u, double* costVector)
{
	...
	double R[]= {10.0, 10.0};
	double diagQ[][STATE_DIM] ={{20, 5, 2, .5},
						{20, 5, 2, .5}};

	for(int whichCost=0; whichCost<numLQRCosts; whichCost++)
	{
		costVector[whichCost]+= dt*R[whichCost]*pow((*u),2);

		for(int i=0; i<STATE_DIM; i++)
			costVector[whichCost] += dt*diagQ[whichCost][i]*pow(x[i],2);
	}
	return costVector;
}
